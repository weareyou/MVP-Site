...
if (!$keyvaultName) {
    ...
    $kvName = "$deploymentId-keyvault"
    $getKeyVault = Get-AzKeyVault -ResourceGroupName $resourceGroupName -VaultName $kvName -ErrorAction SilentlyContinue

} else {
    
    $kvName = "$deploymentId-$keyvaultName"
    $getKeyVault = Get-AzKeyVault -ResourceGroupName $resourceGroupName -VaultName $kvName -ErrorAction SilentlyContinue
}
